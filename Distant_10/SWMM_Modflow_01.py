...
SWMM_path = 'mukilteo_proximate_10%.inp'
...
for i in range(len(HD_TS)):
    ...
    HS_TS_VAR.append(np.nanvar(HD_TS[i]))
np.save('HD_TS_SUM_Proximate_10.npy', HD_TS_SUM)
np.save('HD_TS_VAR_Proximate_10.npy', HS_TS_VAR)

max_index = np.nanargmax(HD_TS_SUM)
min_index = np.nanargmin(HD_TS_SUM)

np.save('HD_TS_MAX_Proximate_10.npy', HD_TS[max_index])
np.save('HD_TS_MIN_Proximate_10.npy', HD_TS[min_index])