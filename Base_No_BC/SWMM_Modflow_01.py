    ...
    for step in sim:
        ...
        if hourly_counter==24:
            ...
            for i in range(len(S_list)):
                ...
                inf_S_list_2[i]=(S_list[i].statistics["infiltration"]/S_area[i]-inf_S_list_1[i])
                inf_S_list_1[i]=S_list[i].statistics["infiltration"]/S_area[i]
...
# np.save('HD_TS_SUM_Base.npy', HD_TS_SUM)
# np.save('HD_TS_VAR_Base.npy', HD_TS_VAR)

# max_index = np.nanargmax(HD_TS_SUM)
# min_index = np.nanargmin(HD_TS_SUM)

# np.save('HD_TS_MAX_Base.npy', HD_TS[max_index])
# np.save('HD_TS_MIN_Base.npy', HD_TS[min_index])
...
data_dict = {month: HD_TS[index] for month, index in months_indices.items()}

np.save('HD_TS_Base_2.5.npy', data_dict)
